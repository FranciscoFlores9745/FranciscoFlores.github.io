...
function blackhole(element) {
    ...
    const cw = w;
    const ch = h;
    const maxorbit = 55; // distance from center
    const centery = ch / 2;
    const centerx = cw / 2;
    ...
    class Star {
        constructor() {
            ...
            const rands = [];
            rands.push(Math.random() * (maxorbit / 5) + 4);
            rands.push(Math.random() * (maxorbit / 3) + maxorbit);

            this.orbital = (rands.reduce((p, c) => p + c,2) / rands.length);
            ...
            this.speed = (Math.floor(Math.random() * 3.5) + 6.5) * Math.PI / 180; // The rate at which this star will orbit
            this.rotation = 0; // current Rotation
            this.startRotation = (Math.floor(Math.random() * 360) + 5) * Math.PI / 180; // Starting rotation

            this.id = stars.length; // This will be used when expansion takes place

            this.collapseBonus = this.orbital - (maxorbit * 0.8); // This "bonus" is used to randomly place some stars outside of the blackhole on hover
            ...
            this.color = 'rgba(200,125,155,' + (1 - ((this.orbital) / 255)) + ')'; // Color the star white, but make it more transparent the further out it is generated

            this.hoverPos = centery + (maxorbit / 2) + this.collapseBonus; // Where the star will go on hover of the blackhole
            this.expansePos = centery + (this.id % 100) * -10 + (Math.floor(Math.random() * 30) + 2); // Where the star will go when expansion takes place
            ...
            if (!expanse && !returning) {
                ...
                if (!collapse) { // not hovered
                    ...
                    if (this.y > this.hoverPos) {
                        this.y -= (this.hoverPos - this.y) / -50;
                    }
                ...
                if (this.y > this.expansePos) {
                    this.y -= Math.floor(this.expansePos - this.y) / -190; // Slower expansion for better visibility
                }
            } else if (returning) {
                // Returning to original orbit slowly
                this.rotation = this.startRotation + (currentTime * this.speed);
                if (Math.abs(this.y - this.originalY) > 2) {
                    this.y += (this.originalY - this.y) / 680; // Much slower return
                } else {
    ...
    centerHover.addEventListener('click', function() {
        ...
        setTimeout(() => {
            ...
            }, 15000); // 8 seconds to return slowly
        }, 10000); // 25 seconds of expansion experience
    });
    ...
    function init() {
        context.fillStyle = 'rgba(28, 26, 26, 1)'; // Initial clear of the canvas
        context.fillRect(0, 0, cw, ch);
        for (let i = 0; i < 7500; i++) { // create 2500 stars
            new Star();
        }
        loop();
    }
